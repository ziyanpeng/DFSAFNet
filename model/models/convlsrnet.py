...
def convnext_small(pretrained=False, in_22k=False, weight_path=None, **kwargs):
    ...
    if pretrained:
        if weight_path is not None:
            print(f"[INFO] Loading ConvNeXt-Small from local path: {weight_path}")
            if weight_path.endswith(".safetensors"):
                # safetensors 读取
                checkpoint = safe_load(weight_path)
                model.load_state_dict(checkpoint, strict=False)
            elif weight_path.endswith(".npz"):
                # npz 文件（如果真有这种，可以自己加解析逻辑）
                import numpy as np
                np_data = np.load(weight_path, allow_pickle=True)
                raise NotImplementedError("Please implement .npz weight loading for your model.")
            else:
                # 传统 pth/pkl 格式
            ...
            url = model_urls['convnext_small_22k'] if in_22k else model_urls['convnext_small_1k']
            print(f"[INFO] Downloading ConvNeXt-Small from {url}")
            checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
            model.load_state_dict(checkpoint["model"], strict=False)
    return model
...
class Model(nn.Module):
    def __init__(self, n_class=6, backbone_name="convnext_small_22k_224",pretrained = True):
        ...
        self.backbone = convnext_small(
            pretrained=True,
            in_22k=True,
            weight_path='pretrain_weights/convnext_small_22k_224.pth')
        # self.backbone = convnext_small(
        #     pretrained=True,
        #     in_22k=True,
        #     weight_path='D:\\pbyy\\GeoSeg-main\\pretrain_weights\\convnext_small_model.safetensors'
        # )
        ...
        #                                       file='D:\\pbyy\\GeoSeg-main\\pretrain_weights\\convnext_small_model.safetensors',
        #                                       custom_load=False))
        self.sps = nn.ModuleList([
