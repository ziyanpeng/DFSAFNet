...
def get_args():
    ...
    arg("-c", "--config_path",default='config/mangrove/base_Dwtres.py', type=Path)
...
def main():
    ...
    # test_size = 1024
    #
    # # Params（完全一致于 Lightning）
    # params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    #
    # # FLOPs（只算前向推理主干 net）
    # train_flops, _ = profile(model.net, inputs=(torch.randn(1, 3, train_size, train_size),), verbose=False)
    # test_flops, _ = profile(model.net, inputs=(torch.randn(1, 3, test_size, test_size),), verbose=False)
    #
    # print(f"Params: {params / 1e6:.3f} M")  # 与 Lightning 一致
    # print(f"Train FLOPs: {train_flops / 1e9:.3f} G @ {train_size}x{train_size}")
    # print(f"Test  FLOPs: {test_flops / 1e9:.3f} G @ {test_size}x{test_size}")
