...
class Evaluator(object):
    ...
    def Precision(self):
        ...
        #precision = tp / (tp + fp)
        precision = np.where((tp + fp) == 0, 0, tp / (tp + fp + eps))  # 避免除零
        return precision

    def Recall(self):
        tp, fp, tn, fn = self.get_tp_fp_tn_fn()
        #recall = tp / (tp + fn)
        recall = np.where((tp + fn) == 0, 0, tp / (tp + fn + eps))  # 避免除零
        return recall
    ...
    def F1(self):
        ...
        # Precision = tp / (tp + fp)
        # Recall = tp / (tp + fn)
        Precision = np.where((tp + fp) == 0, 0, tp / (tp + fp + eps))
        Recall = np.where((tp + fn) == 0, 0, tp / (tp + fn + eps))
        F1 = np.where((Precision + Recall) == 0, 0, (2.0 * Precision * Recall) / (Precision + Recall + eps))
        #F1 = np.where((Precision + Recall) == 0, 0, (2.0 * Precision * Recall) / (Precision + Recall))
        return F1
    ...
    def Intersection_over_Union(self):
        tp, fp, tn, fn = self.get_tp_fp_tn_fn()
        IoU = np.where((tp + fn + fp) == 0, 0, tp / (tp + fn + fp + eps))
        #IoU = tp / (tp + fn + fp)
        return IoU
