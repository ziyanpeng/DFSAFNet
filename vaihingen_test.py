...
def get_args():
    ...
    arg("-c", "--config_path",default="config/vaihingen/DFSAFNet.py" ,type=Path)
    arg("-o", "--output_path", default="test_results/vaihingen", type=Path)
...
def main():
    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            #print(f"Before model: {input['img'].shape}")  # 打印输入尺寸
            raw_predictions = model(input['img'].cuda())
            #print(f"After model: {raw_predictions.shape}")  # 打印输出尺寸
    ...
    print(f"Model checkpoint path: {os.path.join(config.weights_path, config.test_weights_name + '.ckpt')}")
